    ...
    public void run() {
        ...
        while (socket.isConnected()) {
            try {
                ...
                handleIncomingMessage(massageFromClient);
//                broadcastMessage(massageFromClient);
    ...
    private void handleIncomingMessage(String message) {
        if(message.contains("@")) {
            ...
            String[] nameSplit = str[1].split("@");
            String recipient = nameSplit[1];// получатель личного сообщения
            String privateMessage = str[2];// текст личного сообщения
            sendPrivateMessage(recipient, privateMessage); // отправляем личное сообщение
        } else {
    ...
    private void sendPrivateMessage(String recipient, String message) {
        ...
        try {
            bufferedWriter.write("Server: Пользователь '" + recipient + "' не найден");
